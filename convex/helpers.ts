...
export async function validateEnrollment(
  ...
  // Check prerequisites
  const prerequisiteCheck = await validatePrerequisites(
    db,
    studentId,
    section.courseId,
  );
  if (!prerequisiteCheck.isValid) {
    reasons.push(
      `Missing prerequisites: ${prerequisiteCheck.missingPrerequisites.join(", ")}`,
    );
  }
...
export async function calculateAcademicProgress(
  ...
  const creditsByCategory = {
    humanities: {
      required: requirements.requirements.humanities.required,
      completed: 0,
    },
  ...
  for (const enrollment of allEnrollments) {
    if (
      ...
      enrollment.percentageGrade &&
      enrollment.percentageGrade >= 65
    ) {
  ...
  let academicStanding: "good_standing" | "probation" | "suspension" =
    "good_standing";
  ...
  const completionPercentage = Math.round(
    (creditsCompleted / program.totalCredits) * 100,
  );
...
export async function validateGraduationRequirements(
  ...
  const programCourses = await db
    ...
    .withIndex("by_program_required", (q) =>
      q
        .eq("programId", user.studentProfile!.programId)
        .eq("isRequired", true)
        .eq("isActive", true),
    )
    .collect();
...
export async function getAcademicHistory(
  ...
  for (const enrollment of allEnrollments) {
    const periodEnrollments =
      enrollmentsByPeriod.get(enrollment.periodId) || [];
