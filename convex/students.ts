...
export const getMyProgress = query({
  ...
  handler: async (ctx) => {
    ...
    for (const { course, isRequired, category } of programCourses) {
      ...
      const enrollment = completedEnrollments.find(
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      statistics: {
        ...
        gradedCourses: enrollments.filter(
          (e) => e.percentageGrade !== undefined,
        ).length,
        passedCourses: enrollments.filter((e) => e.status === "completed")
          .length,
