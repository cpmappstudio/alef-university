  ...
  handler: async (ctx, args) => {
    ...
    if (args.studentStatus) {
      users = users.filter(
        (user) =>
          user.role === "student" &&
          user.studentProfile?.status === args.studentStatus,
      );
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (targetPeriod) {
      ...
      periodStats = {
        ...
        activeEnrollments: enrollments.filter((e) => e.status === "enrolled")
          .length,
    ...
    const usersByMonth = recentUsers.reduce(
      ...
      {} as Record<
        string,
        { students: number; professors: number; total: number }
      >,
    );
    ...
    return {
      ...
      pendingActions: {
        ...
        gradeSubmissions: periodStats
          ? periodStats.totalSections - periodStats.gradedSections
          : 0,
      },
    };
  },
    ...
    const enrollmentsWithDetails = await Promise.all(
      enrollments.map(async (enrollment) => {
        ...
        return {
          ...
          periodInfo: period
            ? { nameEs: period.nameEs, code: period.code }
            : {},
...
export const createEnrollment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const enrollmentId = await ctx.db.insert("enrollments", {
      ...
      lastGradeUpdate:
        args.percentageGrade !== undefined ? Date.now() : undefined,
...
export const updateEnrollment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (updates.percentageGrade !== undefined) {
      ...
      updatePayload.letterGrade =
        updates.letterGrade || calculateLetterGrade(updates.percentageGrade);
      updatePayload.gradePoints =
        updates.gradePoints || calculateGradePoints(updates.percentageGrade);
      updatePayload.qualityPoints = calculateQualityPoints(
        updatePayload.gradePoints,
        course.credits,
      );
  ...
  handler: async (ctx, args) => {
    ...
    if (sections) {
      throw new ConvexError(
        "Cannot delete a period with associated sections. Please delete sections first.",
      );
    }
...
export const adminUpdateProfessor = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!professor || professor.role !== "professor")
      throw new ConvexError("Professor not found");
  ...
  handler: async (ctx, args) => {
    ...
    if (args.periodId) {
      sections = sections.filter(
        (section) => section.periodId === args.periodId,
      );
    }
    if (args.deliveryMethod) {
      sections = sections.filter(
        (section) => section.deliveryMethod === args.deliveryMethod,
      );
    }
    if (args.status) {
    ...
    if (args.isActive !== undefined) {
      sections = sections.filter(
        (section) => section.isActive === args.isActive,
      );
    }
    ...
    const sectionsWithDetails = await Promise.all(
      sections.map(async (section) => {
        ...
        return {
          ...section,
          courseName: course ? `${course.codeEs} - ${course.nameEs}` : "N/A",
          courseCode: course?.codeEs,
          professorName: professor
            ? `${professor.firstName} ${professor.lastName}`
            : "TBD",
...
export const adminUpdateStudent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (updates.dateOfBirth !== undefined)
      updateObject.dateOfBirth = updates.dateOfBirth;
    if (updates.nationality !== undefined)
      updateObject.nationality = updates.nationality;
    if (updates.documentType !== undefined)
      updateObject.documentType = updates.documentType;
    ...
    if (updates.expectedGraduationDate !== undefined) {
      updateObject.studentProfile.expectedGraduationDate =
        updates.expectedGraduationDate;
    }
