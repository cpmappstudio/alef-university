...
export const getSectionGradeDistribution = query({
  ...
  handler: async (ctx, args) => {
    ...
    const median =
      percentageGrades.length > 0
        ? percentageGrades.sort((a, b) => a - b)[
            Math.floor(percentageGrades.length / 2)
          ]
        : 0;

    const passRate =
      gradedEnrollments.length > 0
        ? (gradedEnrollments.filter((e) => e.percentageGrade! >= 65).length /
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      summary: {
        ...
        coursesCompleted: enrollments.filter((e) => e.status === "completed")
          .length,
  ...
  handler: async (ctx, args) => {
    ...
    for (const gradeSubmission of args.grades) {
      ...
      const enrollment = enrollments.find(
        (e) => e.studentId === gradeSubmission.studentId,
      );

      if (!enrollment) {
        throw new ConvexError(
          `Student ${gradeSubmission.studentId} is not enrolled in this section`,
        );
      }

      // Validate percentage grade
      if (
        gradeSubmission.percentageGrade < 0 ||
        gradeSubmission.percentageGrade > 100
      ) {
        throw new ConvexError(
          `Invalid grade: ${gradeSubmission.percentageGrade}. Must be between 0-100`,
        );
      }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.periodIds) {
      enrollments = enrollments.filter((e) =>
        args.periodIds!.includes(e.periodId),
      );
    }

    if (args.professorId) {
      enrollments = enrollments.filter(
        (e) => e.professorId === args.professorId,
      );
    }
    ...
    const periodStatistics = Array.from(enrollmentsByPeriod.values()).map(
      ({ period, enrollments }) => {
        ...
        const average =
          ...
          grades.length;
        const passRate =
          (enrollments.filter((e: any) => e.percentageGrade >= 65).length /
            enrollments.length) *
          100;
    ...
    return {
      ...
      overallStatistics: {
        ...
        passRate:
          gradedEnrollments.length > 0
            ? Math.round(
                (gradedEnrollments.filter((e) => e.percentageGrade! >= 65)
                  .length /
                  gradedEnrollments.length) *
                  100 *
                  100,
        ...
        withdrawalRate:
          enrollments.length > 0
            ? Math.round(
                (enrollments.filter(
                  (e) => e.status === "withdrawn" || e.status === "dropped",
                ).length /
                  enrollments.length) *
                  100 *
                  100,
      ...
      periodTrends: periodStatistics.sort(
        (a, b) =>
...
export const getIncompleteGradesReport = query({
  ...
  handler: async (ctx, args) => {
    ...
    let incompleteEnrollments = enrollments.filter(
      (enrollment) =>
    ...
    if (args.professorId) {
      incompleteEnrollments = incompleteEnrollments.filter(
        (e) => e.professorId === args.professorId,
      );
    }
        ...
        return {
          ...
          daysOverdue: enrollment.incompleteDeadline
            ? Math.max(
                0,
                Math.floor(
                  (Date.now() - enrollment.incompleteDeadline) /
                    (24 * 60 * 60 * 1000),
                ),
              )
            : null,
    ...
    return {
      ...
      summary: {
        ...
        byStatus: {
          incomplete: incompleteEnrollments.filter(
            (e) => e.status === "incomplete",
          ).length,
          ungraded: incompleteEnrollments.filter(
            (e) => e.status === "enrolled" && e.percentageGrade === undefined,
          ).length,
        },
...
export const validateBulkGrades = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const gradeEntry of args.grades) {
      ...
      if (!studentMatch) {
        errors.push(
          `Student code ${gradeEntry.studentCode} not found in this section`,
        );
        continue;
      }
      ...
      if (gradeEntry.percentageGrade < 0 || gradeEntry.percentageGrade > 100) {
        errors.push(
          `Invalid grade ${gradeEntry.percentageGrade} for student ${gradeEntry.studentCode}. Must be 0-100`,
        );
        continue;
      }
