  ...
  handler: async (ctx, args) => {
    ...
    if (!args.includeInProgress) {
      filteredHistory = filteredHistory
        .map((periodSummary) => ({
          ...periodSummary,
          enrollments: periodSummary.enrollments.filter(
            (e) =>
              e.enrollment.status === "completed" ||
              e.enrollment.status === "failed",
          ),
        }))
    ...
    const allEnrollments = filteredHistory
      .flatMap((p) => p.enrollments.map((e) => e.enrollment))
      .filter((e) => e.countsForGPA);

    const overallGPA = await calculateGPA(ctx.db, allEnrollments);

    // Get academic progress
    const academicProgress = await calculateAcademicProgress(
      ctx.db,
      targetStudentId,
    );

    // Get graduation requirements validation
    const graduationValidation = await validateGraduationRequirements(
      ctx.db,
      targetStudentId,
    );
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      metadata: {
        ...
        totalCredits: enrollmentDetails.reduce(
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      metadata: {
        ...
        withdrawnCourses: enrollments.filter(
          (e) => e.status === "withdrawn" || e.status === "dropped",
        ).length,
        totalCredits: gradeDetails.reduce(
          (sum, g) => sum + (g.course?.credits || 0),
          0,
        ),
      },
    };
    ...
    if (args.requestedFor) {
      if (user.role === "admin" || user.role === "superadmin") {
        ...
        throw new ConvexError(
          "Only administrators can request documents for other users",
        );
      }
    }
  ...
  handler: async (ctx, args) => {
      ...
      documentLogs.map(async (log) => {
        ...
        return {
          ...
          ageInHours: Math.floor(
            (Date.now() - log.generatedAt) / (1000 * 60 * 60),
          ),
        };
      }),
    );
    ...
    return {
      ...
      summary: {
        ...
        completed: documentDetails.filter((d) => d.status === "completed")
          .length,
...
export const generateCompletionCertificate = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      generatedAt: Date.now(),
      language: args.language || "en",
      certificateNumber: `${program.codeEs}-${student.studentProfile.studentCode}-${new Date().getFullYear()}`,
    };
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    if (args.programId) {
      ...
      enrollments = enrollments.filter((e) =>
        programStudentIds.has(e.studentId),
      );
    }
    ...
    const withdrawnEnrollments = enrollments.filter(
      (e) => e.status === "withdrawn" || e.status === "dropped",
    );
    ...
    const averageGPA =
      gradedEnrollments.length > 0
        ? gradedEnrollments.reduce((sum, e) => sum + (e.gradePoints || 0), 0) /
      ...
      analytics: {
        ...
        gradeStatistics: {
          ...
          passRate:
            gradedEnrollments.length > 0
              ? Math.round(
                  (completedEnrollments.length / gradedEnrollments.length) *
                    100 *
                    100,
                ) / 100
              : 0,
