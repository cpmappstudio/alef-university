  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      summary: {
        ...
        totalStudents: sectionDetails.reduce(
          (sum, s) => sum + s.statistics.enrolled,
          0,
        ),
        sectionsWithGrades: sectionDetails.filter(
          (s) => s.statistics.gradesSubmitted,
        ).length,
        sectionsPendingGrades: sectionDetails.filter(
          (s) => !s.statistics.gradesSubmitted,
        ).length,
      },
    };
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      statistics: {
        ...
        enrolledStudents: students.filter(
          (s) => s.enrollment.status === "enrolled",
        ).length,
        gradedStudents: students.filter(
          (s) => s.grade.percentageGrade !== undefined,
        ).length,
        averageGrade:
          students.length > 0
            ? students
              ...
              students.filter((s) => s.grade.percentageGrade !== undefined)
                .length
            : 0,
      },
    };
...
export const updateGrade = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.percentageGrade < 0 || args.percentageGrade > 100) {
      throw new ConvexError(
        `Invalid grade: ${args.percentageGrade}. Must be between 0-100`,
      );
    }
...
export const markGradesSubmitted = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const ungradedStudents = enrollments.filter(
      (e) => e.percentageGrade === undefined,
    );
    if (ungradedStudents.length > 0) {
      throw new ConvexError(
        `Cannot submit grades: ${ungradedStudents.length} students still need grades`,
      );
    }
...
export const getMyTeachingHistory = query({
  ...
  handler: async (ctx, args) => {
          ...
          averageGrade:
            enrollments.length > 0
              ? enrollments
                ...
                enrollments.filter((e) => e.percentageGrade !== undefined)
                  .length
              : 0,
        },
      });
    ...
    let history = Array.from(sectionsByPeriod.values()).sort(
      (a, b) =>
    ...
    return {
      ...
      summary: {
        ...
        totalCoursesDelivered: new Set(
          allSections.map((s) => s.courseId.toString()),
        ).size,
      },
    };
  },
