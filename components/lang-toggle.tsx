  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div
          className={`flex items-center ${showText ? "justify-between w-full px-2" : "justify-center w-8 h-8"} py-1.5 text-left text-sm cursor-pointer ring max-w-3xs ring-muted-foreground rounded-md transition-colors`}
        >
          <div className="flex items-center gap-2">
            <Languages className="h-4 w-4" />
            {showText && (
              <span className="font-medium">{getCurrentLangLabel}</span>
            )}
          </div>
          {showText && <ChevronsUpDown className="ml-auto h-4 w-4" />}
        </div>
