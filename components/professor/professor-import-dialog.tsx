...
export function ProfessorImportDialog() {
  const t = useTranslations("admin.professors.import");
  ...
  const importProfessors = useAction(api.users.importProfessorsFromJSONL);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        try {
          const professor = JSON.parse(lines[i]);
      ...
      // Call import action
      const importResults = await importProfessors({ professors });
